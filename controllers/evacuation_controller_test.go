...
var _ = Describe("Evacuation Controller", func() {
	...
	Describe("EvacuateRunningActualLRP", func() {
		...
		Context("when the update stratergy on the desired LRP is rolling", func() {
			BeforeEach(func() {
				Expect(desiredLRP.UpdateStrategy).To(Equal(models.DesiredLRP_Rolling))
			})
			...
			Context("when the instance is unclaimed", func() {
				...
				Context("without a placement error", func() {
					...
					It("evacuates the LRP", func() {
						...
						_, _, actualLRPKey, actualLRPInstanceKey, actualLrpNetInfo, actualLRPInternalRoutes, actualLRPMetricTags, actualRoutable, actualAvailabilityZone := fakeEvacuationDB.EvacuateActualLRPArgsForCall(0)
						...
						Expect(actualRoutable).To(Equal(false))
						Expect(actualAvailabilityZone).To(Equal(availabilityZone))
					})
		...
		FContext("when the update stratergy on the desired LRP is recreate", func() {
			BeforeEach(func() {
				desiredLRP.UpdateStrategy = models.DesiredLRP_Recreate
			})

			It("does the evacuation dance", func() {})

			It("emits evacuation events", func() {})

			It("removes the evacuation instance", func() {})

			It("emits evacuation removal events", func() {})

			Context("when something errors", func() {
				It("always removes the container", func() {})
			})

			Context("when it can't find the desiredLRP to know the update strategy", func() {
			})
		})
	})
